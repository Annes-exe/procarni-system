...
serve(async (req: Request) => {
  ...
  try {
    ...
    console.log(`[generate-po-pdf] Updating print_date for ${orderId} to ${now}`);
    ...
    if (updateError) {
      console.error('[generate-po-pdf] CRITICAL: Could not update print_date:', updateError);
    } else {
      console.log('[generate-po-pdf] print_date updated successfully');
    ...
    if (!effectiveExchangeRate || effectiveExchangeRate <= 0) {
      console.log('[generate-po-pdf] Exchange rate missing. Fetching daily rate with robust logic...');
      try {
        ...
        if (rateResponse.ok && historyResponse.ok) {
          ...
          if (effective) {
            ...
            if (typeof rate === 'number' && rate > 0) {
              ...
              console.log(`[generate-po-pdf] Fetched effective rate: ${effectiveExchangeRate} para ${baseCurrencyUrl}`);
            } else {
              console.warn('[generate-po-pdf] Invalid rate data computed.');
            }
          }
        } else {
          console.warn('[generate-po-pdf] Failed to fetch rate/history from API.');
        }
      } catch (rateError) {
        console.warn('[generate-po-pdf] Error fetching daily rate:', rateError);
      }
    }

    // Default to 1 if still missing (to avoid division by zero, though this implies USD=VES which is wrong but prevents crash)
    // Or handle it gracefully in the display.
    if (!effectiveExchangeRate) {
      effectiveExchangeRate = 1;
      console.warn('[generate-po-pdf] Warning: No exchange rate available. Defaulting to 1 to prevent errors.');
    }
